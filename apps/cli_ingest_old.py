...
支持：
1. REST 历史数据拉取
2. WebSocket 实时订阅（自动重连 + 心跳保活 + 数据落库）
"""
...
class BinanceWS:
    ...
    async def connect(self):
        ...
        logger.info(f"[Binance WS] 连接: {stream_url}")
    ...
    def handle_message(self, msg):
        ...
        with self.engine.begin() as conn:
            stmt = insert(minute_realtime).values(record)
            stmt = stmt.on_conflict_do_nothing(index_elements=["datetime", "code"])
            conn.execute(stmt)

        logger.info(f"[Binance WS] 保存: {record['code']} {record['datetime']} close={record['close']}")
...
def main():
    ...
    if args.ws_only or (not args.rest_only and not args.ws_only):
        if binance_config.get("websocket", {}).get("enabled", True):
            ...
            ws = BinanceWS(symbols=symbols, interval=interval)
            try:
